...
pub fn handle_get_app_details(app_hash: Address) -> ZomeApiResult<AppBundle> {
    Ok(AppBundle {
        ...
        payment_pref: utils::get_links_and_load_type(
            &app_hash,
...
pub fn handle_get_kv_updates_domain_name() -> ZomeApiResult<Vec<DnsDnaKV>> {
    ...
    for app in all_apps.clone() {
        ...
        let updated_dns: Vec<hc_common::GetLinksLoadElement<DNS>> =
            hc_common::get_links_and_load_type(&app_copy, "new_domain_name_tag".to_string())?;
        // Data refactor
