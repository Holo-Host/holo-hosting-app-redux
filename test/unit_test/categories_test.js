...
module.exports = (scenario) => {
  ...
  test('Category test', async (s, t) => {
    ...
    const app_address = await liza.callSync("app", "provider", "register_app", App_Config_1);
    t.equal(app_address.Ok.length, 46)
    console.log("APP ADDRESS:: ",app_address.Ok);

    const app_address2 = await liza.callSync("app", "provider", "register_app", App_Config_2);
    t.equal(app_address2.Ok.length, 46)
    console.log("APP ADDRESS:: ",app_address2.Ok);

    const result1 = await liza.callSync("app", "categories", "add_category", {category:"Zo", tag:"El", hash:app_address.Ok})
    console.log("add Categories:: ",result1);
    t.equal(result1.Ok, null)

    const result2 = await liza.callSync("app", "categories", "add_category", {category:"Zo", tag:"El", hash:app_address2.Ok})
