...
module.exports = (scenario) => {
  scenario('App Flow Test', async(s, t, {liza}) => {
    ...
    const app_enable = await liza.call("host","enable_app",{app_hash:app_address_1.Ok});
    await liza.call("host","enable_app",{app_hash:app_address_2.Ok});
    console.log("App_enbled: ", app_enable);
    t.equal(app_enable.Ok, null)

    

    const app_list = await liza.call("host","get_enabled_app_list",{});
    console.log("APP List:: ",app_list);
    t.equal(app_list.Ok.length, 2)

    const agent_list = await liza.call("host","get_host_for_app",{app_hash:app_address_2.Ok});
    console.log("Agent List:: ",agent_list.Ok[0]);
    t.equal(agent_list.Ok.length, 1)

    const disable_app_hash = await liza.call("host","disable_app",{app_hash:app_address_2.Ok});
    console.log("App_Disabled: ", app_enable);
    t.equal(disable_app_hash.Ok, null)

    

    const app_list_after_disable = await liza.call("host","get_enabled_app_list",{});
    ...
    */
    const kv_updates = await liza.call("host","get_kv_updates_dna_to_host",{});
    ...
    const kv_updates_2 = await liza.call("host","get_kv_updates_dna_to_host",{});
    ...
    await liza.call("host","kv_updates_host_completed",{kv_bundle:kv_updates_2.Ok.recently_enabled_apps});

    
    /// Calling this fn again should return no new kvs
    const kv_updates_3 = await liza.call("host","get_kv_updates_dna_to_host",{});
    ...
    await liza.call("host","kv_updates_host_completed",{kv_bundle:kv_updates_2.Ok.recently_disabled_apps});


    
    /// Calling this fn again should return no new kvs
    const kv_updates_4 = await liza.call("host","get_kv_updates_dna_to_host",{});
