...
scenario('DNS TO DNA Tests', async(s, t, {liza}) => {
    const Provider_Doc = {
      provider_doc:{
    ...
    //   }
    // }]
    const new_domain_name = await liza.call("provider","get_kv_updates_domain_name",{});
    ...
    // const new_domain_name_again = await liza.call("provider","get_kv_updates_domain_name",{});
    ...
    const updated = await liza.call("provider","kv_updates_domain_name_completed",{kv_bundle:new_domain_name.Ok});
    console.log("Is Updated?:: ",updated);
    t.equal(updated.Ok, null)

    const again_new_domain_name = await liza.call("provider","get_kv_updates_domain_name",{});
